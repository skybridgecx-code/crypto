...
def test_matrix_comparison_artifact_and_reconciliation(tmp_path: Path) -> None:
    ...
    assert comparison["matrix_run_id"] == manifest.matrix_run_id
    assert comparison["row_count"] == len(comparison["rows"])
    ...
    assert fsum(float(row["starting_equity_usd"]) for row in rows) == pytest.approx(
