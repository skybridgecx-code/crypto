...
def _normalize_matrix_trade_ledger(ledger: dict[str, object]) -> dict[str, object]:
...
def test_matrix_trade_ledger_snapshot_and_reconciliation(tmp_path: Path) -> None:
    ...
    assert _normalize_matrix_trade_ledger(matrix_trade_ledger) == _load_snapshot(
        "paper_run_matrix_default.trade_ledger.snapshot.json"
    )
    ...
    manifest_run_ids = [str(entry["run_id"]) for entry in manifest_payload["entries"]]
    assert list(rows_by_run_id) == manifest_run_ids
